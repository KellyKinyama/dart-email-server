...
/// Raw P-256 keypair: 32-byte private scalar and 65-byte uncompressed
/// public key (0x04 || X || Y).
class P256KeyPair {
  final Uint8List privateKey;
  final Uint8List publicKey;
  P256KeyPair({required this.privateKey, required this.publicKey});
}

/// Generate a fresh P-256 keypair.
P256KeyPair p256GenerateKeyPair() {
  final ec = getP256();
  final priv = ec.generatePrivateKey();
  final pubBytes = HEX.decode(priv.publicKey.toHex());
  final privBytes = Uint8List(32);
  final raw = Uint8List.fromList(priv.bytes);
  privBytes.setRange(32 - raw.length, 32, raw);
  return P256KeyPair(
